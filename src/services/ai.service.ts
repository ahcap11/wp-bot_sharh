...
export class AIService {
  ...
  async interpretSalesMessage(
    ...
    const system = [
      ...
      'For sellers, business_type must be the concise activity or sector only. Never include the location/free-zone name, time period, revenue, profit, asking price, or other financial text inside business_type.',
      'Example: “Hamriyah Free Zone oil n gas last yr turnover over 40M aed” means business_type “Oil & Gas”, business_location “Hamriyah Free Zone”, annual_revenue_aed “AED 40,000,000+”.',
      ...
      'Buyer parsing must be conservative with ambiguity. Never assign a number to budget, profit, ROI, or another field merely because it is numeric; use surrounding words and conversation context.',
      'Never relabel EUR, GBP, SAR, or another non-AED currency as AED and never invent an exchange rate. For a seller asking price quoted in USD, the application may convert it to AED using the fixed USD/AED peg; preserve the full range. If a buyer gives only a non-AED budget, leave buyer_budget_aed empty and ask for the maximum in AED.',
      'Understand common buyer phrasing: hands-off, I do not want a job, I do not want to run/manage it, someone else runs it, manager in place, absentee, passive income all describe buyer involvement or management preference.',
      'Understand common money language such as 1.5m, 300k, 2 mil, 300 grand, one and a half million, between 1 and 2 million, per month, p.m., per year, p.a., percent, and pct. Do not invent a period when profit/income/return has no monthly or annual context.',
      'If a profit/income/return amount has no period and context does not make the period explicit, ask whether it is monthly or annual and set hold_funnel=true instead of guessing.',
      'If a buyer with saved criteria sends one bare money amount such as “250k”, do not decide whether it means budget or minimum profit unless the surrounding words make that clear; ask one short disambiguation question.',
      'For a first compact buyer answer to the explicit SHARH schema “sector, maximum budget, minimum annual profit or ROI, passive/manager-run”, two unlabeled money values follow that order: the first is maximum budget and the second is minimum annual profit. Example: “any sector 10000000 100000 passive” means AED 10,000,000 budget, AED 100,000 minimum annual profit, passive required.',
      'Words such as only, must, required, strictly, nowhere else make sector/location a hard constraint; ordinary phrases such as prefer, ideally, looking for should be treated as preferences by the deterministic matcher.',
